...
impl<T: io::Read + io::Write> Xmodem<T> {
    ...
    fn expect_byte_or_cancel(&mut self, byte: u8, expected: &'static str) -> io::Result<u8> {
        let e = self.expect_byte(byte, expected);
        if e.is_err() {
    ...
    pub fn read_packet(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        if buf.len() < 128 {
            return ioerr!(UnexpectedEof, "Recv requires 128 sz bufs");
        }

        let start = self.read_byte(true)?;
        if !self.started {
            self.started = true;
        ...
        match start {
            SOH => {
                let _pkt = self.expect_byte_or_cancel(self.packet, "PktNum")?;
                let _pktcomp = self.expect_byte_or_cancel(255 - self.packet, "pkt comp")?;
                self.inner.read_exact(&mut buf[..])?;
                let rchk = self.read_byte(false)?;
                if rchk != get_checksum(&buf[..]) {
                    self.write_byte(NAK)?;
                    ioerr!(Interrupted, "Invalid checksum")
                } else {
                    self.write_byte(ACK)?;
            ...
            EOT => {
                self.write_byte(NAK)?;
                self.expect_byte_or_cancel(EOT, "EOT2 expected")?;
                self.write_byte(ACK)?;
                self.packet += 1;
                Ok(0)
            }
            _ => ioerr!(InvalidData, "UnexpectedEof"),
        }
    }
    ...
    pub fn write_packet(&mut self, buf: &[u8]) -> io::Result<usize> {
        if !self.started {
            self.expect_byte(NAK, "Expect NAK to start connect")?;
        }
        if buf.len() < 128 && buf.len() != 0 {
            return ioerr!(UnexpectedEof, "Not enough data");
        }
        if buf.len() == 0 {
            self.write_byte(EOT)?;
            self.expect_byte(NAK, "Looking for NAK")?;
            self.write_byte(EOT)?;
            self.expect_byte(ACK, "Looking for NAK")?;
            self.packet += 1;
            return Ok(0);
        }
        if !self.started {
        ...
        let status = self.read_byte(true)?;
        match status {
            ACK => {
                (self.progress)(Progress::Packet(self.packet));
                self.packet += 1;
            ...
            _ => ioerr!(InvalidData, "Pkt_resp not ACK|NAK"),
        }
    }
